...
int main(int argc, char *argv[])
{
    ...
    bool testMode = (argc >= 2 && std::string(argv[1]) == "test");
    if (testMode)
    {
        std::cout << "=== Running in TEST mode ===" << std::endl;
        std::string testsJsonPath = (appRoot / "json" / "tests.json").string();
        std::vector<TestRunner::TestCase> tests = TestRunner::loadTests(testsJsonPath);
        if (tests.empty())
        {
            std::cerr << "No test cases found in " << testsJsonPath << std::endl;
            return 1;
        }
        std::vector<TestRunner::TestResult> results = TestRunner::runAll(tests, appRoot);
